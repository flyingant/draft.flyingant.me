...
const requestAnimationFrame = function(cb: ()=>void) { cb(); return 0; };
...
export default class JSONFormatter {
  ...
  */
  private get isLargeArray(): boolean {
    return (this.isArray && this.config.maxArrayItems!=undefined && this.json.length > this.config.maxArrayItems);
  }
  ...
  */
  private get useToJSON(): boolean | undefined {
  ...
  */
  private get keys(): string[] { 
    if (this.isObject) {
      ...
      if (this.isLargeArray && this.config.maxArrayItems != undefined) {
  ...
  */
  getInlinepreview() {
    if (this.isArray) {
      ...
      if (this.config.hoverPreviewArrayCount != undefined && this.json.length > this.config.hoverPreviewArrayCount) {
      ...
      const ellipsis = this.config.hoverPreviewFieldCount != undefined && keys.length >= this.config.hoverPreviewFieldCount ? '…' : '';
  ...
  */
  appendChildren(animated: boolean = false) {
    ...
    const append = (key: string, index: number) => {
      const range: [number, number] | undefined = ((this.isLargeArray && this.config.maxArrayItems!= undefined) ? [index * this.config.maxArrayItems, Math.min(this.json.length - 1, (index * this.config.maxArrayItems) + (this.config.maxArrayItems - 1))] : undefined);
      const displayKey = ((this.isArrayRange && this.arrayRange!=undefined) ? (this.arrayRange[0] + index).toString() : key);
